		...
		for (int i = 0; i < contour.size(); i++)
		{
			...
			if (i == contour.size() - 1)
			{
				for (int k = 0; k < 2; k++) // 2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222를 3으로 바꿔야 후보 3개 지금은 2로함
				{
					...
					imshow("org_img",read_roi);
					waitKey();

					Mat filter;
					GaussianBlur(read_roi, filter, Size(0, 0), 3);
					addWeighted(read_roi, 5.0, filter, -4.0, 0, read_roi);
					imshow("filter", read_roi);
					waitKey();
					...
					Mat open, close;
					Mat element(1, 4, CV_8U, Scalar(1));
					morphologyEx(thre, open, MORPH_OPEN, element);
					morphologyEx(thre, close, MORPH_CLOSE, element);
					...
					bilateralFilter(read_roi, rest, 5, 75, 75);
					imshow("bilater", rest);
					waitKey();
